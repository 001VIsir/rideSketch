...
## 15. 2026-03-07 全量修复与回归测试记录

> 本节记录本轮“把不能做的功能完善并测试到可用”的全过程，包括问题定位思路、修复过程、验证结果与已知限制。

### 15.1 本轮目标

1. 修复已确认的前后端联调问题，避免“页面可点但功能失败”。
2. 对核心链路执行真实回归测试（后端测试 + 前端构建 + API 冒烟 + 浏览器冒烟）。
3. 将定位思路、修复过程、结果统一沉淀到文档，便于后续追踪。

### 15.2 问题与思路（为什么这样改）

#### 问题A：地图相关能力受高德 JS API 环境影响（前端 key/平台限制）

- **现象**：地图页面在某些环境会出现 JS API 检索/地理编码不可用，进而导致：
  - 地址搜索空结果或失败。
  - 路线规划面板输入文本后无法稳定转成坐标。
- **根因分析**：前端路径过度依赖 `@amap/amap-jsapi-loader`，当 JS API 鉴权或环境受限时没有后备链路。
- **修复策略**：为前端增加“后端 API 回退通道”，保证在 JS API 不稳定时仍可走后端 `MapController` 完成关键流程。

#### 问题B：图案路书过于依赖城市地理编码，外部接口波动会直接失败

- **现象**：`/api/route/pattern` 在地理编码结果空时返回失败，前端功能不可用。
- **根因分析**：后端 `PatternRouteServiceImpl` 对 geocode 结果采取“强依赖 + 直接失败”，没有降级策略。
- **修复策略**：增加服务端兜底中心点（北京），确保图案路书在外部 geocode 抖动下仍可生成。

#### 问题C：导航登录态体验不一致风险（静态/动态引入混用）

- **现象**：存在动态导入与静态导入混用导致的构建告警与行为不一致风险。
- **修复策略**：统一为静态导入调用，简化退出登录流程。

### 15.3 代码变更清单

#### 1) 地址解析回退（前端）

文件：`frontend/src/components/route/RoutePanel.vue`

- 新增 `resolveAddressToPoint(address)`：
  1. 先尝试前端 `utils/amap.ts` 的 `geocode`。
  2. 失败后回退调用后端 `api/map.ts` 的 `geocode`。
  3. 统一返回 `{ lng, lat } | null`。
- `handlePlanRoute` 中起点/终点文本解析改为调用上述回退函数。

**效果**：即使前端 AMap JS 受限，文本输入的路线规划仍可用。

#### 2) 地址搜索回退（前端）

文件：`frontend/src/views/map/MapPage.vue`

- `handleSearch` 中保留原有 JS API 搜索。
- 当 JS API 返回空时，自动回退后端 `searchAddress`。
- 结果沿用既有渲染逻辑，无需改动展示层。

**效果**：降低对单一地图调用链路的依赖，提高搜索可用率。

#### 3) 退出登录流程收敛（前端）

文件：`frontend/src/App.vue`

- 将 `logout` 调用从动态导入改为静态导入。
- `handleLogout` 简化为同步调用 `logout()` 后跳转。

**效果**：行为更稳定，减少构建阶段不必要告警。

#### 4) 图案路书后端兜底（后端）

文件：`src/main/java/org/example/ridesketch/service/impl/PatternRouteServiceImpl.java`

- 在城市地理编码不可用时，不再直接返回失败。
- 增加默认中心点回退：`116.397428, 39.90923`（北京）。
- 保留日志告警，便于后续排查外部 geocode 波动。

**效果**：`/api/route/pattern` 从“外部依赖失败即不可用”提升为“可降级继续可用”。
...
- `vue-tsc -b` 通过
- `vite build` 成功产物输出
- 仅有 Node 版本建议与 chunk 告警，不影响构建成功
...
1. 注册/登录/获取用户
2. 路线规划 `/api/route/plan`
3. 图案路书 `/api/route/pattern`
4. 社区发布/评论/点赞/删除
5. RAG 健康检查
...
- 路线规划状态：`status=1`
- 图案路书状态：`status=1`（修复后验证通过）
- 社区链路（发帖/评论/点赞/删除）可执行并清理

#### D. 前端浏览器冒烟（Playwright）
...
1. 地图搜索与文本地理编码增加后备链路。
2. 图案路书后端加入兜底，避免外部 geocode 抖动导致功能不可用。
3. 前端构建、后端测试、接口冒烟、浏览器冒烟全部跑通。

#### 仍需关注（非本轮阻塞）

1. 运行环境 Node 版本存在建议升级提示（建议升级到 `22.12+` 或 `20.19+`）。
2. 大包 chunk 告警属于性能优化项，不影响功能正确性。
3. 外部地图服务配额/策略仍可能波动，但本轮已完成关键降级兜底。

### 15.6 复现与验收建议

建议验收顺序：

1. 启动依赖：MySQL / Redis / Ollama / 后端 / 前端。
2. 执行：`./mvnw.cmd test`、`frontend npm run build`。
3. 页面验证：登录 -> 地图 -> 文本输入起终点 -> 规划路线。
4. 页面验证：图案路书生成。
5. 页面验证：社区发布/评论/点赞/删除。
